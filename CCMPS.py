...
layout_fractionation = [[sg.Button('Add file...', size = (8,1), key = '-fractionation_add-', disabled = False, enable_events = True, button_color = 'grey'),
                        ...
                        [sg.Column([
                         ...
                         sg.Column([
                             [sg.Button('Fract. Summary', size = (15,1), key = '-fractionation_summary-', disabled = True, enable_events = True, button_color = 'grey')],
...
layout_CCMPS = [[sg.Menu(menu_def, tearoff = False)],
                ...
                title = 'Data Import', size = (620,480)),
                  ...
                  sg.Frame(layout = [
                      [sg.Button('Parameters...', size = (15,1), key = '-classification_parameters-', disabled = False, enable_events = True, button_color = 'black'),
                       ...
                       sg.Button('ML Validation', size = (15,1), key = '-classification_validation-', disabled = True, enable_events = True),
                          ...
                          [sg.Frame(layout = [
                              [sg.Button('Predict Proteome!', size = (32,1), key = '-statistic_predict-', disabled = True, enable_events = True, button_color = 'dark blue')],
                                      ...
                                      [sg.Button('Export Prediction', size = (15,1), key = '-statistic_export-', disabled = True, enable_events = True, button_color = 'black'),
                                       sg.Button('Import Prediction', size = (15,1), key = '-statistic_import-', disabled = True, enable_events = True, button_color = 'black')],
                                      [sg.HSep()],
                                      [sg.VPush()],
                ...
                [sg.Frame(layout = [
                    [sg.Frame(layout = [
                      ...
                      sg.Column(layout = [
                          [sg.Button('Load preset-list', size = (32,1), key = '-marker_preset-', disabled = False, enable_events = True, button_color = 'dark blue')],
                          [sg.HSep()],
                          [sg.Text('Fract. Key:\t'),
                          ...
                          sg.Frame(layout = [
                              ...
                              sg.Frame(layout = [
                                  ...
                                  [sg.HSep()],
                                  [sg.Button('Class Heatmaps', size = (15,1), key = '-class_heatmap-', disabled = True, enable_events = True),
                                   sg.Button('Class Reorg. Plots', size = (15,1), key = '-class_reorganization-', disabled = True, enable_events = True)],
...
while True:
    ...
    # if event_CCMPS == '-classification_statistics-':
    #     results = MOP_stats.stats_exec3('deep', learning_xyz, NN_params, fract_data, tp_data, fract_marker, fract_conditions, fract_std)
    #     #window_CCMPS['-classification_comparison-'].Update(disabled = False)
    #     window_CCMPS['-status_statistics-'].Update('done!')
    
    # if event_CCMPS == '-classification_statistics_rough-':
    #     results = MOP_stats.stats_exec3('rough', learning_xyz, NN_params, fract_data, tp_data, fract_marker, fract_conditions, fract_std)
    
    # if event_CCMPS == '-classification_statistics-':
    #     results = MOP_stats.stats_exec2(learning_xyz, NN_params, tp_data, fract_marker)
    #     window_CCMPS['-classification_comparison-'].Update(disabled = False)
    #     window_CCMPS['-status_statistics-'].Update('done!')
    ...
    if event_CCMPS == 'New':
        ...
        if sure == 'Yes':
            ...
            #action.fract_buttons(window_CCMPS, False)
            #action.tp_buttons(window_CCMPS, False)
            window_CCMPS['-marker_fractkey-'].Update(values = ['[IDENTIFIER]'] + list(fract_info))
            #window_CCMPS['-classification_MOP-'].Update(disabled = True)
            #window_CCMPS['-classification_SVM-'].Update(disabled = True)
            #window_CCMPS['-classification_statistics-'].Update(disabled = True)
            #window_CCMPS['-classification_comparison-'].Update(disabled = True)
            #window_CCMPS['-status_statistics-'].Update('missing')
            #window_CCMPS['-status_comparison-'].Update('missing')
            #window_CCMPS['-export_statistics-'].Update(disabled = True)
            #window_CCMPS['-export_statistics-'].Update(disabled = True)
           # window_CCMPS['-marker_tpkey-'].Update(values = ['[IDENTIFIER]'] + tp_info.columns.tolist())
        else:
            pass
